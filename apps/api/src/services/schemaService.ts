...
import { schemaMetaSchema, SchemaMetadata } from '@sqlcat/types';
...
export async function getSchemaMetadata(schemaId: number): Promise<SchemaMetadata> {
  return withDbClient(async (client: PoolClient) => {
    ...
    const tablesRes = await client.query<{ table_name: string }>('SELECT table_name FROM information_schema.tables WHERE table_schema = $1 ORDER BY table_name', [row.slug]);
    const tables: SchemaMetadata['tables'] = [];
    for (const table of tablesRes.rows) {
      const columnsRes = await client.query<{ column_name: string; data_type: string }>(
        'SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = $1 AND table_name = $2 ORDER BY ordinal_position',
        [row.slug, table.table_name],
      );
      tables.push({
